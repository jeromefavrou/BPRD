...
public class Rive : MonoBehaviour
{
    ...
    // Génération des sommets et des UVs
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                if( y > (int)_gen.limite.getLimiteYMax( (uint)(x) )+1 )
                    h = 0.5f;
                if( y == (int)_gen.limite.getLimiteYMax( (uint)(x) )+1 )
                    h = 0.0f;
                ...
                // Ajout des triangles // && y < (int)_gen.limite.getLimiteYMax( (uint)(x) )-2 && y >=(int)_gen.limite.getLimiteYMin( (uint)(x) )+1
